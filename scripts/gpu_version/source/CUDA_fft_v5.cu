...
// --- Butterfly kernel ottimizzato per grandi array ---
__global__ void butterfly_kernel(cuFloatComplex* data, size_t n, size_t len) {
    extern __shared__ cuFloatComplex sdata[];

    size_t tid = threadIdx.x;
    size_t pairs_per_block = blockDim.x;
    size_t block_start = (blockIdx.x * pairs_per_block) * 2;

    // Copia dati globali in shared memory
    if (block_start + tid < n) sdata[tid] = __ldg(&data[block_start + tid]);
    if (block_start + tid + pairs_per_block < n) sdata[tid + pairs_per_block] = __ldg(&data[block_start + tid + pairs_per_block]);
    __syncthreads();

    if (tid < pairs_per_block && block_start + tid + pairs_per_block < n) {
        float x = 2.0f * tid / float(len);
        float s = sinf(M_PI * x);
        float c = cosf(M_PI * x);
        cuFloatComplex w = make_cuFloatComplex(c, -s);

        cuFloatComplex u = sdata[tid];
        cuFloatComplex v = cuCmulf(sdata[tid + pairs_per_block], w);

        sdata[tid] = cuCaddf(u, v);
        sdata[tid + pairs_per_block] = cuCsubf(u, v);
    }
    __syncthreads();

    // Scrivi di nuovo in globale
    if (block_start + tid < n) data[block_start + tid] = sdata[tid];
    if (block_start + tid + pairs_per_block < n) data[block_start + tid + pairs_per_block] = sdata[tid + pairs_per_block];
}

// --- Host GPU FFT wrapper ---
std::vector<std::complex<float>> fft_gpu(
    ...
    CHECK_CUDA_ERROR(cudaMemcpyToSymbol(M_PI, &h_PI, sizeof(float)));
    ...
    for (size_t len = 2; len <= n; len <<= 1) {
        size_t pairs = n / 2;
        int threads = (threadsPerBlock < pairs) ? threadsPerBlock : pairs;
        int num_blocks = (pairs + threads - 1) / threads;
        size_t shared_mem_bytes = 2 * threads * sizeof(cuFloatComplex);

        butterfly_kernel<<<num_blocks, threads, shared_mem_bytes>>>(d_data, n, len);
        CHECK_CUDA_ERROR(cudaGetLastError());
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());
    }
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[RESULTS] TotalTime: " << total_exec_ms << "ms\n";
    return 0;
}
